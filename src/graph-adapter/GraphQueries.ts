...
/** @returns id of the send message generated by the service */
export const sendMessage = async (chatId: string, message: string): Promise<string> => {
  const chatMessage = {
    body: {
      content: message
    }
  ...
  const response = await Providers.client.api(`/chats/${chatId}/messages`).post(chatMessage);
  console.log('GraphQuery sendMessage response:', response);
  return response.id;
};
